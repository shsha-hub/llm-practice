...
def fallback_answer(
    ...
    if not response.confident or not response.results:
        return (
            "관련 규칙을 충분히 확신할 수 없습니다."
            "품목의 재질, 오염 여부와 현재 지역을 더 구체적으로 입력하고 실제 지자체 안내를 확인하세요."
        )
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "제공된 부산광역시·부산진구 공식 자료만 근거로 답한다."
                    "품목 분류, 배출 방법, 주의사항, 근거 ID를 간결하게 포함한다."
                    "규칙이 서로 충돌하거나 일정 근거가 없으면 추측하지 말고 부산진구 자원순환과 확인을 권한다."
                    "공동주택은 관리사무소 기준을 우선한다고 밝힌다."
                ),
            },
            {
                "role": "user",
